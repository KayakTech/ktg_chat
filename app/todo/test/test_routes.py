...
from app.todo.factories import TodoFactory
...
class TestTodo(BaseTest):
    def test_create_todo(self):
        ...
        response = self.client.post("/todos/", json=data)
        ...
        user_todo = services.get_todos(self.db, user.id)
    ...
    def test_get_todo(self):
        ...
        TodoFactory(created_by=user)
        TodoFactory(created_by=another_user)
        
        all_todos = self.db.query(Todo)       
        user_todos = services.get_todos(self.db, user.id)

        assert user_todos.count() == 1
        assert all_todos.count() >= 2


        response = self.client.get(f"/todos/")
        assert response.status_code == status.HTTP_200_OK
